...
3. JS `updatePnlAndCheckExits(position, …)` for each:
   ...
   - `OUT_OF_RANGE` if `minutes_out_of_range >= outOfRangeWaitMinutesAbove` (above range) or `outOfRangeWaitMinutesBelow` (below range)
